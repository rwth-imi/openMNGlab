...
def median_RMS(sweeps, center_sweep_idx, latency_slope, latency, radius, window_size = 0.002, sampling_rate = 20000):
	...
	for r, (sweep_idx, sweep) in zip(range(-radius, radius + 1), enumerate(sweeps[center_sweep_idx - radius : center_sweep_idx + radius])):
		...
		t = latency + r * latency_slope
		# calculate the window borders
		t_min_idx = max(floor((t - (window_size / 2)) * sampling_rate), 0)
		t_max_idx = max(min(floor((t + (window_size / 2)) * sampling_rate), len(sweep.raw_signal)), 0)
